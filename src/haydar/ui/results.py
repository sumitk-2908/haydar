...
def _prepare_snippet(snippet: str) -> str:
    """Collapse whitespace so the snippet renders cleanly on a single line.

    The snippet is rendered as plain text (``Qt.PlainText``), so no escaping or
    markup is applied -- file-derived content can never be interpreted as HTML.
    """
